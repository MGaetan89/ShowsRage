...
class OkHttpGlideModule : com.bumptech.glide.integration.okhttp.OkHttpGlideModule() {
    override fun registerComponents(context: Context?, glide: Glide?) {
        val preferences = PreferenceManager.getDefaultSharedPreferences(context)
        val selfSignedCertificate = preferences.getBoolean("self_signed_certificate", false)
        val client = SickRageApi.getInstance().getOkHttpClient(selfSignedCertificate)
